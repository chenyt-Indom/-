...
4.5. 如果距离>1000km：默认使用飞机作为交通工具（用户特殊说明除外），优先选择上午出发的航班，确保到达后有充足时间
        ...
        if is_self_drive:
            ...
            if transport_info:
                ...
                if dest_hub.get("has_hub"):
                    transport_section += f"\n【目的地枢纽】{dest_hub['note']}"
    transport_section += f"""
【交通选择原则】根据距离选择：≤5km步行，≤30km公交/地铁/打车，≤100km地铁/城际，≤300km高铁/动车，≤800km高铁优先，800-1000km飞机/高铁，>1000km默认飞机（用户特殊说明除外）
第一天必须包含从{departure_city}出发前往{dest}的交通规划，最后一天必须包含从{dest}返回{departure_city}的交通规划。
